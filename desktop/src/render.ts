...
export interface ScreenElements {
  ...
  readonly preparingStatus: HTMLElement
  readonly preparingBar: HTMLElement
  readonly preparingStages: HTMLElement
  readonly cancelButton: HTMLButtonElement
  readonly cancelNote: HTMLElement

  readonly failed: HTMLElement
  readonly failedHeading: HTMLElement
  readonly failedHint: HTMLElement
  readonly failedCode: HTMLElement
  readonly failedStage: HTMLElement
  readonly failedDetail: HTMLElement
  readonly retryButton: HTMLButtonElement
  readonly diagnosticsButton: HTMLButtonElement

  readonly reconnecting: HTMLElement
  readonly reconnectingHeading: HTMLElement
  readonly reconnectingHint: HTMLElement

  readonly ready: HTMLElement
}

const RECONNECT_HINT: Record<'token_missing' | 'engine_restarted', string> = {
...
function renderStageItem(stage: StageProgress): string {
...
function renderPreparing(
  state: Extract<UiState, { kind: 'preparing' }>,
  els: ScreenElements,
): void {
...
function renderReconnecting(
  state: Extract<UiState, { kind: 'reconnecting' }>,
  els: ScreenElements,
): void {
 ...
 */
export function render(state: UiState, els: ScreenElements): void {
  ...
  if (state.kind === 'preparing') renderPreparing(state, els)
  if (state.kind === 'failed') renderFailed(state, els)
  if (state.kind === 'reconnecting') renderReconnecting(state, els)
}
