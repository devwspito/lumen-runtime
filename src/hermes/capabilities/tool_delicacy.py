...
# safent-ads companion (024/T091/T194): NINGUNA tool del companion es MFA-tier.
# Sus escrituras son de dos clases y ninguna sube gasto por si sola:
#   - propose_*/withdraw_proposal crean una fila pendiente en el datastore del
#     companion que un humano aprueba DESPUES por otro canal (Telegram/REST);
#   - apply_defensive_action solo admite `lower_budget|pause`
#     (safent-ads mcp/presentation/args.py::ApplyDefensiveActionKindArgs) y pasa
#     por el chokepoint del companion (topes, freno, guardarrailes, broker).
# Por eso el eje `approval` del policy_overlay se respeta para ellas: el
# propietario pidio "pausar y bajar" autonomo; cualquier SUBIDA solo existe
